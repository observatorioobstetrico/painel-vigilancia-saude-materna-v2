...
raca_muni_agr <- raca_muni %>%
  ...
  summarise(nasc_raca = sum(NASCIDOS)) |> clean_names()
...
# total de nascimentos coincide com o total de nascidos vivos do painel de Indicadores Obstétricos
...
CONSPRENAT_agr_antigo <-  read_delim(("bases/CONSPRENAT_muni.csv"),
                                     delim = ";", escape_double = FALSE, trim_ws = TRUE) |>
...
CONSPRENAT_antigo <-  read_delim(("bases/CONSPRENAT_muni.csv"),
                                 delim = ";", escape_double = FALSE, trim_ws = TRUE) |>
  rename(CODMUNRES = Municipio,
         ANO = Ano,
         NASC = Nascidos)
...
base_1_antiga <- read.csv('Base_1.csv')
...
write.csv(dados_final_2,'Base_1_2012-2022.csv')
