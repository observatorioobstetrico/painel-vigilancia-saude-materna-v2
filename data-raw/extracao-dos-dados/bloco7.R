...
df_fetais_totais <- fetch_datasus(
  year_start = 2012,
  year_end = 2022,
...
df_neonat_total2 <- process_sim(df_neonat_total, municipality_data = T) |>
  select(
...
df_neonat_total3 <- df_neonat_total2  |>
  mutate(
    ano = substr(DTOBITO, 1, 4),
    codmunres = as.numeric(CODMUNRES),
...
df_nascidos_total <- fetch_datasus(
  year_start = 2012,
  year_end = 2022,
