...
mod_bloco_7_ui <- function(id) {
  ...
  tagList(
    div(
      ...
      h2(tags$b(HTML("Mortalidade neonatal e fetal: série histórica"), htmlOutput(ns("titulo_localidade"), inline = TRUE)), style = "padding-left: 0.4em"),
    ...
    data7_resumo <- reactive({
      ...
      bloco7 |>
        ...
        dplyr::summarise(
          ...
          mort_neonat_menos1500 = round(sum(obitos_27dias_menos1500)/sum(nascidos) *1000, 2),
          mort_neonat_precoc_menos1500 = round(sum(obitos_6dias_menos1500)/sum(nascidos) *1000, 2),
          mort_neonat_tardia_menos1500 = round(sum(obitos_7_27dias_menos1500)/sum(nascidos) *1000, 2),
          mort_neonat_1500_1999 = round(sum(obitos_27dias_1500_1999)/sum(nascidos) *1000, 2),
          mort_neonat_precoc_1500_1999 = round(sum(obitos_6dias_1500_1999)/sum(nascidos) *1000, 2),
          mort_neonat_tardia_1500_1999 = round(sum(obitos_7_27dias_1500_1999)/sum(nascidos) *1000, 2),
          mort_neonat_2000_2499 = round(sum(obitos_27dias_2000_2499)/sum(nascidos) *1000, 2),
          mort_neonat_precoc_2000_2499 = round(sum(obitos_6dias_2000_2499)/sum(nascidos) *1000, 2),
          mort_neonat_tardia_2000_2499 = round(sum(obitos_7_27dias_2000_2499)/sum(nascidos) *1000, 2),
          mort_neonat_mais2500 = round(sum(obitos_27dias_mais2500)/sum(nascidos) *1000, 2),
          mort_neonat_precoc_mais2500 = round(sum(obitos_6dias_mais2500)/sum(nascidos) *1000, 2),
          mort_neonat_tardia_mais2500 = round(sum(obitos_7_27dias_mais2500)/sum(nascidos) *1000, 2),
    ...
    data7_resumo_referencia <- reactive({
      bloco7 |>
        ...
        dplyr::summarise(
          ...
          mort_neonat_tardia_mais2500 = round(sum(obitos_7_27dias_mais2500)/sum(nascidos) *1000, 2),
          obitos_fetais = sum(obitos_fetais_mais_22sem),
          taxa_mort_fetal = round(sum(obitos_fetais_mais_22sem)/sum(sum(nascidos),sum(obitos_fetais_mais_22sem)) *1000, 2)
        ) |>
        dplyr::ungroup()
    })
